...
const fetchSeniors = async () => {
  loading.value = true;
  try {
    const response = await apiClient.get('/olders/');
...
const searchSeniors = async () => {
  ...
  loading.value = true;
  try {
    const response = await apiClient.get(`/olders/search?name=${searchName.value}`);
...
const saveSenior = async () => {
  try {
    if (isEditing.value) {
      // 更新 (PUT)
      await apiClient.put('/olders/', currentSenior.value);
    } else {
      // 新增 (POST)
      await apiClient.post('/olders/', currentSenior.value);
    }
...
const deleteSenior = async (id) => {
  ...
  try {
    await apiClient.delete(`/olders/${id}`);
    alert('删除成功！');
    fetchSeniors(); // 刷新列表
  } catch (error) {
